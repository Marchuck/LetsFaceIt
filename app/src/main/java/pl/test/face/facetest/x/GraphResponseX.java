 ...
 */
public class GraphResponseX {
    private final HttpURLConnection connection;
    private final JSONObject graphObject;
    private final JSONArray graphObjectArray;
    private final FacebookRequestErrorX error;
   ...
   public GraphResponseX(
     ...
     */
    public final FacebookRequestErrorX getError() {
        return error;
    }
     ...
     */
    public GraphRequestX getRequestForPagedResults(PagingDirection direction) {
        String link = null;
        if (graphObject != null) {
        ...
        GraphRequestX pagingRequest;
        try {
            pagingRequest = new GraphRequestX(request.getAccessToken(), new URL(link));
    ...
    static List<GraphResponseX> fromHttpConnection(
            HttpURLConnection connection,
    ...
    static List<GraphResponseX> createResponsesFromStream(
            InputStream stream,
            HttpURLConnection connection,
            GraphRequestBatchX requests
    ...
    static List<GraphResponseX> createResponsesFromString(
        ...
        List<GraphResponseX> responses = createResponsesFromObject(
                connection,
                requests,
                resultObject);
        Logger.log(
    ...
    private static List<GraphResponseX> createResponsesFromObject(
            HttpURLConnection connection,
            List<GraphRequestX> requests,
            Object object
    ) throws FacebookException, JSONException {
        int numRequests = requests.size();
        List<GraphResponseX> responses = new ArrayList< >(numRequests);
            ...
            try {
                ...
                responses.add(
                        new GraphResponseX(
                                request,
                                connection,
                                new FacebookRequestErrorX(connection, e)));
            } catch (IOException e) {
                responses.add(
                        new GraphResponseX(
                                request,
    ...
    private static GraphResponseX createResponseFromObject(
            GraphRequestX request,
            HttpURLConnection connection,
            Object object,
        ...
        if (object instanceof JSONObject) {
            ...
            FacebookRequestErrorX error =
                    FacebookRequestErrorX.checkResponseAndCreateError(
                            jsonObject,
                            originalResult,
                            connection);
            AccessTokenX accessTokenX  = request.getAccessToken();
            boolean isCurrentAccessToken = accessTokenX != null && accessTokenX.equals(AccessTokenX.getCurrentAccessToken());
            ...
            if (error != null) {
                ...
                return new GraphResponseX(request, connection, error);
            }
            ...
            if (body instanceof JSONObject) {
                return new GraphResponseX(request, connection, body.toString(), (JSONObject)body);
            } else if (body instanceof JSONArray) {
                return new GraphResponseX(request, connection, body.toString(), (JSONArray)body);
            }
        ...
        if (object == JSONObject.NULL) {
            return new GraphResponseX(request, connection, object.toString(), (JSONObject)null);
        } else {
    ...
    static List<GraphResponseX> constructErrorResponses(
            List<GraphRequestX> requests,
            HttpURLConnection connection,
            FacebookException error) {
        int count = requests.size();
        List<GraphResponseX> responses = new ArrayList< >(count);
        for (int i = 0; i < count; ++i) {
            GraphResponseX response = new GraphResponseX(
                    requests.get(i),
                    connection,
                    new FacebookRequestErrorX(connection, error));
