...
final class ProfileManagerX {
    ...
    private static volatile ProfileManagerX instance;

    private final LocalBroadcastManager localBroadcastManager;
    private final ProfileCacheX profileCache;
    private ProfileX currentProfile;


    ProfileManagerX(
            LocalBroadcastManager localBroadcastManager,
            ProfileCacheX profileCache) {
    ...
    static ProfileManagerX getInstance() {
        if (instance == null) {
            synchronized (ProfileManagerX.class) {
                if (instance == null) {
                    Context applicationContext = FacebookSdk.getApplicationContext();
                    LocalBroadcastManager localBroadcastManager = LocalBroadcastManager.getInstance(
                            applicationContext);

                    instance = new ProfileManagerX(localBroadcastManager, new ProfileCacheX());
                }
            }
        }
        return instance;
    ...
    ProfileX getCurrentProfile() {
        return currentProfile;
    }

    boolean loadCurrentProfile() {
        ProfileX profile = profileCache.load();
    ...
    void setCurrentProfile(ProfileX currentProfile) {
        setCurrentProfile(currentProfile, true);
    }

    private void setCurrentProfile(ProfileX currentProfile, boolean writeToCache) {
        ProfileX oldProfile = this.currentProfile;
