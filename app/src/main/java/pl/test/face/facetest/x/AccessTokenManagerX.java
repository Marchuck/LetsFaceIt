...
final class AccessTokenManagerX {
    ...
    private static volatile AccessTokenManagerX instance;

    private final LocalBroadcastManager localBroadcastManager;
    private final AccessTokenCacheX accessTokenCache;
    private AccessTokenX currentAccessToken;
    private AtomicBoolean tokenRefreshInProgress = new AtomicBoolean(false);
    private Date lastAttemptedTokenExtendDate = new Date(0);

    AccessTokenManagerX(LocalBroadcastManager localBroadcastManager,
                       AccessTokenCacheX accessTokenCache) {
    ...
    static AccessTokenManagerX getInstance() {
        if (instance == null) {
            synchronized (AccessTokenManagerX.class) {
                if (instance == null) {
                    Context applicationContext = com.facebook.FacebookSdk.getApplicationContext();
                    LocalBroadcastManager localBroadcastManager = LocalBroadcastManager.getInstance(
                            applicationContext);
                    AccessTokenCacheX accessTokenCache = new AccessTokenCacheX();

                    instance = new AccessTokenManagerX(localBroadcastManager, accessTokenCache);
                }
            }
        }
    ...
    AccessTokenX getCurrentAccessToken() {
        return currentAccessToken;
    }

    boolean loadCurrentAccessToken() {
        AccessTokenX accessToken = accessTokenCache.load();
    ...
    void setCurrentAccessToken(AccessTokenX currentAccessToken) {
        setCurrentAccessToken(currentAccessToken, true);
    }

    private void setCurrentAccessToken(AccessTokenX currentAccessToken, boolean saveToCache) {
        AccessTokenX oldAccessToken = this.currentAccessToken;
        ...
        if (saveToCache) {
            if (currentAccessToken != null) {
                ...
                Utility.clearFacebookCookies(com.facebook.FacebookSdk.getApplicationContext());
            }
        }
    ...
    private void sendCurrentAccessTokenChangedBroadcast(AccessTokenX oldAccessToken,
                                                        AccessTokenX currentAccessToken) {
    ...
    private static GraphRequestX createGrantedPermissionsRequest(
            AccessTokenX accessToken,
            GraphRequestX.Callback callback
    ) {
        Bundle parameters = new Bundle();
        return new GraphRequestX(
                accessToken,
    ...
    private static GraphRequestX createExtendAccessTokenRequest(
            AccessTokenX accessToken,
            GraphRequestX.Callback callback
    ) {
        Bundle parameters = new Bundle();
    ...
    private void refreshCurrentAccessTokenImpl() {
        final AccessTokenX accessToken = currentAccessToken;
        if (accessToken == null) {
            return;
        }
        ...
        GraphRequestBatchX batch = new GraphRequestBatchX(
                createGrantedPermissionsRequest(accessToken, new GraphRequestX.Callback() {
                    @Override
                ...
                createExtendAccessTokenRequest(accessToken, new GraphRequestX.Callback() {
                    @Override
                    public void onCompleted(GraphResponseX response) {
        ...
        batch.addCallback(new GraphRequestBatchX.Callback() {
            @Override
            public void onBatchCompleted(GraphRequestBatchX batch) {
                ...
                try {
                    ...
                    AccessTokenX newAccessToken = new AccessTokenX(
