 ...
 */
public final class ShareDialogX
        extends FacebookDialogBase<ShareContent, Sharer.Result>
        implements Sharer {
    public static final String TAG = ShareDialogX.class.getSimpleName();
    ...
    public static void show(
        ...
        Log.i(TAG, "show: ");
        new ShareDialogX(activity).show(shareContent);
    }
    ...
    public static void show(
            ...
            final ShareContent shareContent) {
        new ShareDialogX(fragment).show(shareContent);
    }
     ...
     * @param activity Activity to use to share the provided content.
     */
    public ShareDialogX(Activity activity) {
        super(activity, DEFAULT_REQUEST_CODE);
        Log.i(TAG, "ShareDialogX: 0");
     ...
     * @param fragment Fragment to use to share the provided content.
     */
    public ShareDialogX(Fragment fragment) {
        super(fragment, DEFAULT_REQUEST_CODE);
        Log.i(TAG, "ShareDialogX: 1");
        ShareInternalUtility.registerStaticShareCallback(DEFAULT_REQUEST_CODE);
    }

    // for ShareDialog use only
    ShareDialogX(Activity activity, int requestCode) {
        super(activity, requestCode);
        Log.i(TAG, "ShareDialogX: 2");
        ShareInternalUtility.registerStaticShareCallback(requestCode);
    }

    // for ShareDialog use only
    ShareDialogX(Fragment fragment, int requestCode) {
        super(fragment, requestCode);
        Log.i(TAG, "ShareDialogX: 3");
        ShareInternalUtility.registerStaticShareCallback(requestCode);
    }
    ...
    protected void registerCallbackImpl(
        ...
        Log.i(TAG, "registerCallbackImpl: 0");
        Log.i(TAG, "registerCallbackImpl: " + getRequestCode());

        ShareInternalUtility.registerSharerCallback(
                      getRequestCode(), callbackManager, callback);

       /* ShareInternalUtility.registerSharerCallback(
                getRequestCode(), callbackManager, new FacebookCallback<Result>() {
                    @Override
                    ...
                    public void onCancel() {
                        Log.i(TAG, "onCancel: 22");

                    }

                    @Override
                    public void onError(FacebookException error) {
                        Log.i(TAG, "onError: 33 " + error.getMessage());
                    }
                });*/
    }
    ...
    public boolean getShouldFailOnDataError() {
        Log.i(TAG, "getShouldFailOnDataError: 0");
        return this.shouldFailOnDataError;
    }

    @Override
    public void setShouldFailOnDataError(boolean shouldFailOnDataError) {
        Log.i(TAG, "setShouldFailOnDataError: 0");
    ...
    protected AppCall createBaseAppCall() {
        Log.i(TAG, "createBaseAppCall: ");
        return new AppCall(getRequestCode());
    }

    @Override
    protected List<ModeHandler> getOrderedModeHandlers() {
        Log.i(TAG, "getOrderedModeHandlers: ");
    ...
    private class NativeHandler extends ModeHandler {
        ...
        public boolean canShow(final ShareContent content) {
            return content != null && ShareDialogX.canShowNative(content.getClass());
        }
    ...
    private class WebShareHandler extends ModeHandler {
        ...
        @Override
        public boolean canShow(final ShareContent content) {
            return (content != null) && ShareDialogX.canShowWebTypeCheck(content.getClass());
        }
    ...
    private void logDialogShare(Context context, ShareContent content, Mode mode) {
        ...
        for (String key : parameters.keySet()) {
            Log.d(TAG, key + " is a key for " + parameters.get(key));
        }
